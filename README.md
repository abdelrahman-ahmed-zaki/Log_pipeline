...
log_pipeline/ │ ├── log.py ├── README.md │ ├── flume-conf/ │ └── taildir-to-hdfs.conf │ └── images/ ├── architecture.png ├── flowchart.png └── dashboard.png

⚙️ Project Workflow

Generate random log data using Python.
Apache Flume monitors the log file.
Flume transfers logs into Hadoop HDFS.
...
mkdir -p ~/log_pipeline cd ~/log_pipeline
...
mkdir ~/flume-conf nano ~/flume-conf/taildir-to-hdfs.conf
...
df_split = df.withColumn( "date", split(df["value"], " ").getItem(0) ).withColumn( "time", split(df["value"], " ").getItem(1) ).withColumn( "level", split(df["value"], " ").getItem(2) )
...
levels = [row["level"] for row in pdf] counts = [row["count"] for row in pdf]

1️⃣4️⃣ Visualize Data

import matplotlib.pyplot as plt

plt.figure(figsize=(8,5)) plt.bar(levels, counts) plt.title("Log Levels Distribution") plt.xlabel("Log Level") plt.ylabel("Count") plt.show()
